...
(() => {
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j) => {
        let localJ = null;
        try {
          const ls = localStorage.getItem('image_slots_state');
          if (ls) localJ = JSON.parse(ls);
        } catch(e) {}
        const base = Object.assign({}, j || {}, localJ || {});
      ...
      .catch(() => {
        try {
          const ls = localStorage.getItem('image_slots_state');
          if (ls) slots = Object.assign({}, JSON.parse(ls), slots);
        } catch(e) {}
      })
  ...
  function setSlot(id, val) {
    ...
    try {
      localStorage.setItem('image_slots_state', JSON.stringify(slots));
    } catch(e) {}
    subs.forEach((fn) => fn());
  ...
  const stylesheet =
    ...
    ':host([bare][data-filled]) .frame{background:transparent;overflow:visible}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '<div class="ctl" popover="manual" data-dc-edit-transparent><button data-act="replace" title="Re-upload or replace image">📷 Re-upload Image</button>' +
        '  <button data-act="edit" title="Reframe image">✂️ Crop</button></div>' +
    ...
    _applyView() {
      ...
      if (g && g.iw && g.ih) {
        ...
        if (this.parentElement && !this.parentElement.hasAttribute('data-fixed-aspect')) {
          this.parentElement.style.aspectRatio = String(ar);
        }
      }
